 ...
 *   node scripts/fetch_drive_assets.mjs --token "$GOOGLE_OAUTH_TOKEN" --only cartography
 *
...
function parseArgs(argv) {
  const a = { apiKey: process.env.GOOGLE_API_KEY || null, token: process.env.GOOGLE_OAUTH_TOKEN || null, only: null, force: false, dryRun: false };
...
if (!assets.length) {
  console.error(`error: no assets matched --only ${args.only}`);
  process.exit(2);
}
