...
const STEPS_EN: StepContent[] = [
  ...
  { title: 'Timeline Territory Map', desc: 'The Timeline Territory Map lets you explore real geographic territories for 22 historical periods on a live OpenStreetMap map. Select a topic from the left panel and the map flies to that territory, placing annotated markers for key locations.', tips: ['22 historical territories from Ancient Egypt to the Yugoslav Wars', 'Click any marker to read a location note', 'Topics grouped by era — Ancient, Medieval, Early Modern, Modern', 'Available to all users — no plan required'], linkLabel: 'Open Territory Map' },
];
...
const STEPS_ES: StepContent[] = [
  ...
  { title: 'Mapa de Territorios Históricos', desc: 'El Mapa de Territorios te permite explorar territorios geográficos reales de 22 períodos históricos en un mapa OpenStreetMap en vivo. Selecciona un tema del panel izquierdo y el mapa vuela a ese territorio colocando marcadores anotados.', tips: ['22 territorios históricos desde el Antiguo Egipto hasta las Guerras Yugoslavas', 'Haz clic en cualquier marcador para leer una nota de ubicación', 'Temas agrupados por era — Antiguo, Medieval, Moderno Temprano, Moderno', 'Disponible para todos los usuarios'], linkLabel: 'Abrir Mapa de Territorios' },
];
...
const STEPS_RU: StepContent[] = [
  ...
  { title: 'Карта Исторических Территорий', desc: 'Карта Территорий позволяет исследовать реальные географические территории 22 исторических периодов на живой карте OpenStreetMap. Выберите тему из левой панели, и карта перелетит к этой территории с аннотированными маркерами.', tips: ['22 исторические территории — от Древнего Египта до Югославских войн', 'Нажмите на любой маркер, чтобы прочитать заметку о месте', 'Темы сгруппированы по эпохам — Древний мир, Средние века, Раннее Новое время, Современность', 'Доступно всем пользователям — без ограничений по плану'], linkLabel: 'Открыть Карту Территорий' },
];
...
const STEPS_MK: StepContent[] = [
  ...
  { title: 'Карта на Историски Територии', desc: 'Картата на Територии ти овозможува да истражуваш реални географски територии за 22 историски периоди на жива OpenStreetMap карта. Избери тема од левиот панел и картата лета до таа територија поставувајќи анотирани маркери за клучни локации.', tips: ['22 историски територии — од Античко Египет до Југословенските Војни', 'Кликни на кој било маркер за да прочиташ белешка за локацијата', 'Теми групирани по епохи — Античко, Средновековно, Рано Модерно, Модерно', 'Достапно за сите корисници — без ограничување по план'], linkLabel: 'Отвори Карта на Територии' },
];
