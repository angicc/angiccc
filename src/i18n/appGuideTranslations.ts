...
const STEPS_EN: StepContent[] = [
  ...
  { title: 'Timeline Territory Map', desc: 'Explore real geographic territories for 22 historical periods on a live OpenStreetMap. Switch between Explore mode (toggle layers, read annotated markers), Story mode (auto-animated tour of each key location), and Quiz mode (110-question bank — 5 unique questions per territory with full explanations).', tips: ['Requires Pro Learner or Master Student plan', 'Switch between Explore, Story, and Quiz modes', '110-question bank — 5 unique questions per territory, no repeats per session', 'Quiz XP capped at 500 per session to keep it fair'], linkLabel: 'Open Territory Map' },
];
...
const STEPS_ES: StepContent[] = [
  ...
  { title: 'Mapa de Territorios Históricos', desc: 'Explora territorios geográficos reales de 22 períodos históricos en un mapa OpenStreetMap en vivo. Cambia entre modo Explorar (filtra capas, lee marcadores anotados), modo Historia (tour animado de ubicaciones clave) y modo Quiz (110 preguntas — 5 únicas por territorio con explicaciones completas).', tips: ['Requiere plan Pro Learner o Master Student', 'Cambia entre modos Explorar, Historia y Quiz', 'Banco de 110 preguntas — 5 únicas por territorio, sin repeticiones por sesión', 'XP del quiz limitado a 500 por sesión para mantener la equidad'], linkLabel: 'Abrir Mapa de Territorios' },
];
...
const STEPS_RU: StepContent[] = [
  ...
  { title: 'Карта Исторических Территорий', desc: 'Исследуйте реальные географические территории 22 исторических периодов на живой карте OpenStreetMap. Переключайтесь между режимом Исследования (фильтрация слоёв, аннотированные маркеры), режимом Истории (автоматический анимированный тур по ключевым местам) и режимом Викторины (110 вопросов — 5 уникальных на территорию с полными объяснениями).', tips: ['Требуется план Pro Learner или Master Student', 'Переключайтесь между режимами: Исследование, История и Викторина', '110 вопросов — 5 уникальных на территорию, без повторов за сессию', 'XP от викторины ограничен до 500 за сессию для справедливости'], linkLabel: 'Открыть Карту Территорий' },
];
...
const STEPS_MK: StepContent[] = [
  ...
  { title: 'Карта на Историски Територии', desc: 'Истражувај реални географски територии за 22 историски периоди на жива OpenStreetMap карта. Префрлај се меѓу режим Истражување (филтрирај слоеви, читај анотирани маркери), режим Приказна (автоматска анимирана тура на клучни локации) и режим Квиз (110 прашања — 5 единствени по територија со целосни објаснувања).', tips: ['Потребен план Pro Learner или Master Student', 'Префрлај се меѓу режимите: Истражување, Приказна и Квиз', '110 прашања — 5 единствени по територија, без повторување по сесија', 'XP од квизот е ограничен на 500 по сесија за правичност'], linkLabel: 'Отвори Карта на Територии' },
];
