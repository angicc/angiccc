  ...
  return (
    <AppShell>
      ...
      <div className="max-w-3xl mx-auto">
        <div className="mb-6"><h1 className="font-heading text-3xl font-bold">{quiz.title}</h1><p className="text-muted-foreground mt-1 text-sm">{quiz.questions.length} {t.quiz_questions} · {quiz.xpPerCorrect} {t.quiz_xp_earned}</p></div>
        ...
        {phase === 'done' && (
          <Card>
            <CardContent className="flex flex-col items-center text-center gap-4 py-10">
              ...
              <div className="text-muted-foreground">{correct} / {quiz.questions.length} {t.quiz_correct_label}</div>
              <Badge variant={score >= quiz.passingScore ? 'default' : 'secondary'}>{score >= quiz.passingScore ? t.quiz_passed : t.quiz_failed}</Badge>
              {xpAmt > 0 && <div className="text-sm text-primary font-medium">+{xpAmt} {t.quiz_xp_earned}!</div>}
              <div className="flex gap-3 mt-2">
                <Button variant="outline" className="gap-2" onClick={start}><RotateCcw className="w-4 h-4" />{t.quiz_retake}</Button>
                <Button className="gap-2" onClick={() => navigate('/eras')}>{t.btn_back}<ArrowRight className="w-4 h-4" /></Button>
              </div>
            </CardContent>
          </Card>
        )}
