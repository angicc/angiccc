import { LayoutDashboard, Terminal, Users, Shield, FlaskConical, Webhook, MessageSquare, Trophy, User, Settings, Zap, Activity, ChevronDown, ChevronRight } from 'lucide-react';
import { useState } from 'react';
import { useAuditStore } from '../../store/auditState';

export type View = 'input' | 'terminal' | 'leads' | 'dns' | 'abtesting' | 'webhooks' | 'replies' | 'leaderboard' | 'profile' | 'settings';
...
type NavSection = {
  label: string;
  items: { id: View; icon: React.ElementType; label: string; badge?: string }[];
};
...
export default function Sidebar({ activeView, onViewChange }: Props) {
  ...
  return (
    <aside className="w-16 lg:w-60 flex-shrink-0 bg-slate-950 border-r border-white/[0.06] flex flex-col h-full overflow-hidden">
      ...
      {/* Bottom status */}
      <div className="px-3 pb-4 hidden lg:block flex-shrink-0">
