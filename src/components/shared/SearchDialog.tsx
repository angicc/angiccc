...
export function SearchDialog({ open, onClose }: Props) {
  ...
  const lessons = q.length < 2 ? [] : LESSONS.filter(l =>
    l.title.toLowerCase().includes(q) || l.subtitle.toLowerCase().includes(q)
  ).slice(0, 5);

  const events = q.length < 2 ? [] : TIMELINE.filter(e =>
      ...
      <DialogContent className="p-0 gap-0 max-w-lg overflow-hidden">
        <div className="flex items-center gap-3 px-4 py-3 border-b border-border">
          ...
          <Input
            ...
            placeholder="Search lessons, events…"
        ...
        {(lessons.length > 0 || events.length > 0) && (
          <div className="max-h-96 overflow-y-auto divide-y divide-border">
            {lessons.length > 0 && (
              <div className="p-2">
                <p className="text-xs text-muted-foreground px-2 py-1 font-medium uppercase tracking-wide">Lessons</p>
                {lessons.map(l => {
                  const era = ERAS.find(e => e.id === l.eraId);
                  return (
            ...
            {events.length > 0 && (
              <div className="p-2">
                <p className="text-xs text-muted-foreground px-2 py-1 font-medium uppercase tracking-wide">Timeline Events</p>
                {events.map(e => (
                  <button key={e.id} onClick={() => go('/timeline')}
                    ...
                    <Badge variant="outline" className={`text-xs shrink-0 ${eraColor[e.eraId]}`}>{e.category}</Badge>
                  </button>
                ))}
              </div>
            )}
        ...
        {q.length >= 2 && lessons.length === 0 && events.length === 0 && (
          <div className="py-12 text-center text-muted-foreground text-sm">No results for "{query}"</div>
        )}
