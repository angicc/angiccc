...
interface Props { title?: string; description?: string; requiredPlan?: 'pro' | 'master'; compact?: boolean; }
  ...
  return (
    <Card className="border-primary/40 bg-primary/5">
      <CardContent className="flex flex-col items-center text-center gap-4 py-10">
        ...
        <Button onClick={() => nav('/pricing')} className="gap-2"><Zap className="w-4 h-4" />{requiredPlan === 'master' ? 'Upgrade to Master Student' : 'Upgrade to Pro Learner'}</Button>
      </CardContent>
    </Card>
  );
}
